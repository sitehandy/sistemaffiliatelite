...
if (!is_null($ref))
{
    ...
    $result = mysql_query("SELECT * FROM sales WHERE refid = '$ref' AND kaedahpembayaran = 'PPL' AND statuspelanggan = '".AFF_AS_STATUSPENDING."' AND ipaddress = '$clientip' ORDER BY date, time desc", $database_connection) or die ('Database Connection Error');
    ...
    if (mysql_num_rows($result) > 0)
    {
        ...
        mysql_query("UPDATE sales SET statuspelanggan = '$statuspelanggan' WHERE refid = '$ref' AND kaedahpembayaran = 'PPL' AND ipaddress = '$clientip' AND statuspelanggan = '".AFF_AS_STATUSPENDING."'", $database_connection) or die ('Database Update Error');
    }
    else {
