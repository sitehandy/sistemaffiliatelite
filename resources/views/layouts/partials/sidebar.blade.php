        ...
        @if(auth()->user()->role?->name === 'admin')
            ...
            <div class="mb-6">
                ...
                <ul class="space-y-1">
                    ...
                    <li>
                        <a href="{{ route('admin.upgrade.index') }}" @click="sidebarOpen = false" class="flex items-center px-3 py-2 rounded hover:bg-slate-700 {{ request()->routeIs('admin.upgrade.*') ? 'bg-slate-700' : '' }}">
                            <svg class="w-5 h-5 mr-3 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path>
                            </svg>
                            <span>System Upgrade</span>
                            @inject('upgradeService', 'App\Services\UpgradeService')
                            @if($upgradeService->needsUpgrade())
                                <span class="ml-auto bg-red-500 text-white text-xs px-2 py-0.5 rounded-full">New</span>
                            @endif
                        </a>
                    </li>
                </ul>
