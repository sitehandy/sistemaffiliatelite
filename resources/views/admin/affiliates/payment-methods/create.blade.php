...
@section('page-title', 'Add Payment Method for ' . $affiliate->name)
...
@section('content')
    ...
    <div class="max-w-2xl">
        <form method="POST" action="{{ route('admin.affiliates.payment-methods.store', $affiliate) }}" class="bg-white rounded-lg shadow p-6" x-data="{ type: '{{ old('type', 'bank') }}' }">
            @csrf
