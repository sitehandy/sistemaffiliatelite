...
@section('page-title', 'Create Affiliate Program')

@section('content')
    <div class="max-w-3xl">
        <form method="POST" action="{{ route('admin.programs.store') }}" class="bg-white rounded-lg shadow p-6">
            @csrf
            ...
            <div class="grid grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <select name="program_type" id="program_type" required
                        ...
                        <option value="sale" {{ old('program_type') === 'sale' ? 'selected' : '' }}>Pay Per Sale</option>
                        <option value="lead" {{ old('program_type') === 'lead' ? 'selected' : '' }}>Pay Per Lead</option>
                        <option value="view" {{ old('program_type') === 'view' ? 'selected' : '' }}>Pay Per View</option>
                    </select>
                    @error('program_type')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="visibility" id="visibility" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                        <option value="open" {{ old('visibility') === 'open' ? 'selected' : '' }}>Open (Public)</option>
                        <option value="hidden" {{ old('visibility') === 'hidden' ? 'selected' : '' }}>Hidden (Invite Only)</option>
                    </select>
                    @error('visibility')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
            ...
            <div class="grid grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <select name="commission_type" id="commission_type" required
                        ...
                        <option value="flat" {{ old('commission_type') === 'flat' ? 'selected' : '' }}>Flat Rate</option>
                    </select>
                ...
                <div>
                    ...
                    <input type="number" name="commission_amount" id="commission_amount" value="{{ old('commission_amount') }}" step="0.01" min="0" required
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 @error('commission_amount') border-red-500 @enderror">
            ...
            <div class="mb-6">
                ...
                <input type="url" name="default_url" id="default_url" value="{{ old('default_url') }}" placeholder="https://your-website.com/landing-page"
            ...
            <div class="mb-6">
                ...
                <div class="border border-gray-300 rounded-lg p-4 max-h-60 overflow-y-auto">
                    ...
                    @empty
                        <p class="text-sm text-gray-500">No products available. <a href="{{ route('admin.products.create') }}" class="text-blue-600">Create a product first.</a></p>
                    @endforelse
                </div>
            </div>
            ...
            <div class="flex justify-end space-x-3">
                ...
                <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Create Program</button>
            </div>
        </form>
    </div>
@endsection
