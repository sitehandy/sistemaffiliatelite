        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-200">
                @forelse($commissions as $commission)
                    <tr>
                        <td class="px-6 py-4 text-sm">{{ $commission->user->name }}</td>
                        ...
                        <td class="px-6 py-4 text-right text-sm">
                            @if($commission->status === 'pending')
                                <form method="POST" action="{{ route('admin.commissions.approve', $commission) }}" class="inline">@csrf<button type="submit" class="text-green-600 hover:text-green-900 mr-2">Approve</button></form>
                                <form method="POST" action="{{ route('admin.commissions.reject', $commission) }}" class="inline">@csrf<button type="submit" class="text-red-600 hover:text-red-900">Reject</button></form>
                            @endif
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="6" class="px-6 py-4 text-center text-gray-500">No commissions found.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
