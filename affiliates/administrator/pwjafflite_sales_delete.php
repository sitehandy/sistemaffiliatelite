...
if (($_REQUEST['delete']) && ($_REQUEST['validation'] == $_SESSION['aff_valid_admin']))
{
    ...
    if($_POST['commited'] == 'yes')
    {
        ...
        if($errorMsg == '')
        {
            mysql_query("DELETE FROM sales WHERE idsales = ".$_REQUEST['delete']." LIMIT 1", $database_connection) or die("Database DELETE Error");
    ...
    <br />
        <form action="pwjafflite_sales_delete.php?delete=<?=$_REQUEST['delete']?>&validation=<?=$_SESSION['aff_valid_admin']?>" method="post" ENCTYPE="multipart/form-data">
            <div align="center">
                <table cellspacing="1" class="SA_adminarea_statisticbox">
                    <tr>
                        <td colspan="3" class="SA_adminarea_statisticbox_header"><?=AFF_A_SALESRESET?> - Sales ID [<?=$_REQUEST['delete']?>]</td>
                    </tr>
                    <tr>
                        <td colspan="3" class="SA_adminarea_statisticbox_row1">
                            <br /><?=AFF_A_1SALESRESETCONFIRM?><br /><br />
