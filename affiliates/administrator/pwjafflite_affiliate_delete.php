...
{    
    ...
    if($_POST['commited'] == 'yes')
    {
        ...
        if($errorMsg == '')
        {
            mysql_query("DELETE FROM affiliates WHERE refid = '".$_REQUEST['delete']."' LIMIT 1", $database_connection) or die("Database DELETE Error");
            aff_redirect('pwjafflite_admin_affiliates.php');
    ...
    <br />
        <form action="pwjafflite_affiliate_delete.php?delete=<?=$_REQUEST['delete']?>&validation=<?=$_SESSION['aff_valid_admin']?>" method="post" ENCTYPE="multipart/form-data">
            <div align="center">
                <table cellspacing="1" class="SA_adminarea_statisticbox">
                    <tr>
                        <td colspan="3" class="SA_adminarea_statisticbox_header"><?=AFF_A_AFFILIATEDELETE?> - <?=$_REQUEST['delete']?></td>
                    </tr>
                    <tr>
                        <td colspan="3" class="SA_adminarea_statisticbox_row1">
                            <br /><div align="justfy"><?=AFF_A_AFFILIATEDELETECONFIRM?></div><br /><br />
