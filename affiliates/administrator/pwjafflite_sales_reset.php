...
if (($_REQUEST['agen']) && ($_REQUEST['delete'] == 'allsalesrecords') && ($_REQUEST['validation'] == $_SESSION['aff_valid_admin']))
{
    ...
    if($_POST['commited'] == 'yes')
    {
        ...
        if($errorMsg == '')
        {
            mysql_query("DELETE FROM sales WHERE refid = '".$_REQUEST['agen']."'", $database_connection) or die("Database Connect Error");
            aff_redirect('pwjafflite_affiliate_statistic.php');
        }
        if($errorMsg != '')
        {
    ...
    <br />
        <form action="pwjafflite_sales_reset.php?agen=<?=$_REQUEST['agen']?>&delete=allsalesrecords&validation=<?=$_SESSION['aff_valid_admin']?>" method="post" ENCTYPE="multipart/form-data">
            <div align="center">
                <table cellspacing="1" class="SA_adminarea_statisticbox">
                    <tr>
                        <td colspan="3" class="SA_adminarea_statisticbox_header"><?=AFF_A_SALESRESET?> - <?=$_REQUEST['agen']?></td>
                    </tr>
                    <tr>
...
else if (($_REQUEST['agen']) && (($_REQUEST['delete'] == AFF_AS_STATUSPENDING) || ($_REQUEST['delete'] == AFF_AS_STATUSVERIFIED) || ($_REQUEST['delete'] == AFF_AS_STATUSPAID)) && ($_REQUEST['validation'] == $_SESSION['aff_valid_admin']))
{
    // Sahkan proses penghapusan data
    $errorMsg = '';
    ...
    if($_POST['commited'] == 'yes')
    {
        ...
        if($errorMsg == '')
        {
            mysql_query("DELETE FROM sales WHERE refid = '".$_REQUEST['agen']."' AND statuspelanggan = '$_REQUEST[delete]'", $database_connection) or die("Database Connect Error");
    ...
    <br />
        <form action="pwjafflite_sales_reset.php?agen=<?=$_REQUEST['agen']?>&delete=<?=$_REQUEST['delete']?>&validation=<?=$_SESSION['aff_valid_admin']?>" method="post" ENCTYPE="multipart/form-data">
            <div align="center">
                <table cellspacing="1" class="SA_adminarea_statisticbox">
                    <tr>
                        <td colspan="3" class="SA_adminarea_statisticbox_header"><?=AFF_A_SALESSTATUSRESET?> - <?=$_REQUEST['delete']?> - <?=$_REQUEST['agen']?></td>
                    </tr>
                    <tr>
                        <td colspan="3" class="SA_adminarea_statisticbox_row1">
